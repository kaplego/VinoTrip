 ...
 * ==               LOCALITE               == *
 * ========================================== */
const vignobleSelect = document.getElementById("vignoble"), localiteControl = document.getElementById("localite-control"), localiteSelect = document.getElementById("localite");
function updateLocalite() {
    const vignoble = vignobleSelect.value;
    localiteControl.classList.toggle("hidden", localiteSelect.querySelectorAll(`option[data-vignoble='${vignoble}']`)
        .length === 0);
    localiteSelect.querySelectorAll(`option`).forEach((localite) => {
        localite.classList.toggle("hidden", localite.getAttribute("data-vignoble") != vignoble &&
            localite.value !== "null");
    });
 ...
 * ==                ETAPES                == *
 * ========================================== */
const etapesTitre = document.getElementById("etapes-titre"), addEtape = document.getElementById("add-etape"), etapes = document.getElementById("etapes"), maxEtapes = 5, minEtapes = 1;
...
function deleteEtape(ev) {
    if (ev.currentTarget instanceof HTMLElement) {
        document
            .getElementById("etape-" + ev.currentTarget.getAttribute("data-etape"))
            ?.remove();
        document
            .getElementById("hebergement-" + ev.currentTarget.getAttribute("data-etape"))
            ?.remove();
    }
    updateEtapeIndexes();
    updateEtapeCount();
